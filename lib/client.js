...
function Client(options) {
  ...
  this._errorRetryTime = options.errorRetryTime || HConstants.DEFAULT_ERROR_RETRY_TIME;
  this._errorRetryNumber = options.errorRetryNumber || HConstants.DEFAULT_ERROR_RETRY_NUMBER;

  this._clientPause = options.clientPause || HConstants.DEFAULT_HBASE_CLIENT_PAUSE;
  this._clientRetriesNumber = options.clientRetriesNumber || HConstants.DEFAULT_HBASE_RETRIES_NUMBER;
  this._clientRetriesCount = 0;
...
Client.prototype._action = function (method, tableName, obj, callback, retry) {
  if (retry == null) {
    retry = 0;
  }
  retry++;
  ...
  self.locateRegion(tableName, row, true, function (err, location) {
    if (err || !location) {
      if (retry <= self._errorRetryNumber) {
        setTimeout(function() {
          self._action(method, tableName, obj, callback, retry);
        }, self._errorRetryTime * retry);
        return;
      }
      return callback(err);
    }
    ...
    self.getRegionConnection(location.getHostname(), location.getPort(), function (err, server) {
      ...
      server[method](location.getRegionInfo().getRegionName(), obj, function (err, value) {
        if (err && retry <= self._errorRetryNumber) {
          setTimeout(function() {
            self._action(method, tableName, obj, callback, retry);
          }, self._errorRetryTime * retry);
          return;
...
Client.prototype.processBatch = function (tableName, workingList, callback) {
  ...
  ep.fail(function (err) {
    if (self._clientRetriesNumber > self._clientRetriesCount) {
      self._clientRetriesCount++;
      setTimeout(self.processBatch.bind(self), self._clientPause, tableName, workingList, callback);
    } else {
      self._clientRetriesCount = 0;
      callback(err);
    }
  });
 ...
 * put rows into table
 * @param  {String|Buffer} tableName
 * @param  {Array} rows [{row: 'aaabbbcccddd', 'ts': timestamp, 'f:name': 'xa', 'f:age': 1}]
 */
      ...
      function (err, regionInfoRow) {
        ...
        if (Bytes.equals(parentTable, HConstants.META_TABLE_NAME)) {
          ...
          // self.prefetchRegionCache(tableName, HConstants.EMPTY_START_ROW, function (err, count) {
            ...
            self.emit(eventName);
            debug('[%s, startRow:%s] prefetchRegionCache %d locations',
...
Client.prototype.getRegionConnection = function (hostname, port, callback) {
  ...
  server.once('close', function () {
    ...
    delete self.cachedRegionLocations[HConstants.META_TABLE_NAME.__key]
    ...
    if (server.state == 'connecting') {
      setTimeout(function(){self.getRegionConnection(hostname, port, function(){});}, self._timeoutTime);
    }
  });
};
